...
type PostionList = Vec<Range<usize>>;
...
pub fn get_args() -> MyResult<Config> {
    ...
    Ok(Config {
        ...
        delimeter: *delim_bytes.first().unwrap(),
        extract,
    })
}
            ...
            Ok(file) => match &config.extract {
                Fields(fields_pos) => {
                    ...
                    for record in reader.records() {
                        let record = record?;
                        wtr.write_record(extract_fields(&record, fields_pos))?;
                    }
                }
                Bytes(byte_pos) => {
...
fn parse_index(input: &str) -> Result<usize, String> {
    // let value_error = || format!("illegal list value: \"{}\"",
    // input);
...
fn parse_pos(range: &str) -> MyResult<PostionList> {
    let range_re = Regex::new(r"^(\d+)-(\d+)$").unwrap();

    range
        .split(',') // 0개 이상의 , 가 있는 str
        .into_iter()
        .map(|val| {
...
fn extract_fields(record: &StringRecord, field_pos: &[Range<usize>]) -> Vec<String> {
    field_pos
        ...
        .map(String::from) // Iterator::map to turn &str values into String values.
        .collect()
}
